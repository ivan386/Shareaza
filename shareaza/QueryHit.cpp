...
CQueryHit::CQueryHit(PROTOCOLID nProtocol, CGUID* pSearchID)
{
	m_pNext = NULL;
	...
	if ( pSearchID != NULL )
		...
		m_pSearchID = (CGUID&)GUID_NULL;
	
	m_nProtocol		= nProtocol;
	m_pClientID		= (CGUID&)GUID_NULL;
...
CQueryHit* CQueryHit::FromPacket(CG2Packet* pPacket, int* pnHops)
{
	...
	try
	{
		...
		while ( pPacket->ReadPacket( szType, nLength, &bCompound ) )
		{
			...
			{
				pPacket->Read( &pClientID, GUID_SIZE );
				bClientID	= TRUE;
				pIncrID		= pClientID;
			}
		...
		pPacket->Read( &pSearchID, GUID_SIZE );
...
CQueryHit* CQueryHit::FromPacket(CEDPacket* pPacket, SOCKADDR_IN* pServer, CGUID* pSearchID)
{
	CQueryHit* pFirstHit	= NULL;
	CQueryHit* pLastHit		= NULL;
	CHashED2K oED2K;
		 ...
		 pPacket->m_nType == ED2K_S2CG_SEARCHRESULT )
	{
		...
		while ( nCount-- > 0 && pPacket->GetRemaining() >= ED2K_HASH_SIZE + 10 )
		{
	...
	{
		if ( pPacket->GetRemaining() < ED2K_HASH_SIZE + 1 ) return NULL;
		pPacket->Read( oED2K );
		BYTE nCount = pPacket->ReadByte();
...
void CQueryHit::ParseAttributes(CGUID* pClientID, CVendor* pVendor, BYTE* nFlags, BOOL bChat, BOOL bBrowseHost)
{
...
void CQueryHit::ReadG2Packet(CG2Packet* pPacket, DWORD nLength)
{
	...
	if ( ! m_oSHA1.IsValid() && ! m_oTiger.IsValid() && ! m_oED2K.IsValid() && ! m_oBTH.IsValid() ) AfxThrowUserException();
}
...
void CQueryHit::ReadEDAddress(CEDPacket* pPacket, SOCKADDR_IN* pServer)
{
	...
	m_pClientID.m_d[ 0 ] = pServer->sin_addr.S_un.S_addr;
	m_pClientID.m_d[ 1 ] = htons( pServer->sin_port );
	m_pClientID.m_d[ 2 ] = nAddress;
	m_pClientID.m_d[ 3 ] = m_nPort;
...
{
	if ( m_bPreview && m_oSHA1.IsValid() && m_sPreview.IsEmpty() )
	{
	...
	if ( m_nProtocol == PROTOCOL_ED2K )
	{
		if ( m_bPush == TS_TRUE )
		{
			m_sURL.Format( _T("ed2kftp://%lu@%s:%i/%s/%I64i/"),
				m_pClientID.m_d[ 2 ],
				(LPCTSTR)CString( inet_ntoa( (IN_ADDR&)m_pClientID.m_d[ 0 ] ) ),
				m_pClientID.m_d[ 1 ],
	...
	if ( m_nIndex == 0 || m_oTiger.IsValid() || m_oED2K.IsValid() || Settings.Downloads.RequestHash )
	{
		if ( m_oSHA1.IsValid() )
		{
...
void CQueryHit::Serialize(CArchive& ar, int nVersion)
{
	if ( ar.IsStoring() )
	{
		ar.Write( &m_pSearchID, GUID_SIZE );
		
		ar << m_nProtocol;
		ar.Write( &m_pClientID, GUID_SIZE );
	...
	{
		ar.Read( &m_pSearchID, GUID_SIZE );
		
		if ( nVersion >= 9 ) ar >> m_nProtocol;
		ar.Read( &m_pClientID, GUID_SIZE );
