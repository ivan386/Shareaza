...
void CMD5::Add(LPCVOID pData, DWORD nLength)
{
	...
	if ( (m_nCount[0] += ((DWORD)nLength << 3)) < ((DWORD)nLength << 3) )
		m_nCount[1]++;
	m_nCount[1] += ((DWORD)nLength >> 29);
	...
	if ( nLength >= partLen )
	{
		CopyMemory( &m_nBuffer[index], pInput, partLen );
		Transform( m_nBuffer );
	...
	CopyMemory( &m_nBuffer[index], &pInput[i], nLength - i );
}

//////////////////////////////////////////////////////////////////////
// CMD5 transform block

void CMD5::Transform(BYTE* pBlock)
{
	DWORD a = m_nState[0], b = m_nState[1], c = m_nState[2], d = m_nState[3], x[16];
...
void CMD5::Encode(BYTE* pOutput, DWORD* pInput, DWORD nLength)
{
	unsigned int i, j;
...
void CMD5::Decode(DWORD* pOutput, BYTE* pInput, DWORD nLength)
{
	unsigned int i, j;
