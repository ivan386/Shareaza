...
BOOL CScheduleTaskDlg::OnInitDialog() 
{
	...
	else	//We are editing an existing schedule task, getting values from it
	{
		...
		case 0:
			//Should never happen
			theApp.Message( MSG_ERROR, _T("Scheduler dialog box encountered an invalid task.") );
		...
		if (m_wndTypeSel.GetCurSel() == BANDWIDTH_REDUCED_SPEED)
		{
	...
	if( m_bSpecificDays ) EnableDaysOfWeek(true);
	else EnableDaysOfWeek(false);


	UpdateData(FALSE);
	if (m_wndTypeSel.GetCurSel() == BANDWIDTH_REDUCED_SPEED) OnBnClickedToggleBandwidth();
	return FALSE;
}
...
void CScheduleTaskDlg::OnOK() 
{
	...
	else
	{
		...
		if( !m_nDays )
		{
			AfxMessageBox(IDS_SCHEDULER_SELECTADAY,MB_OK);
			return;
		}
