...
BOOL CChildWnd::Create(UINT nID, HWND phParent)
{
	if ( phParent == NULL ) return FALSE;
	...
	m_nResID = nID;
	LoadString( strCaption, m_nResID );
	CMDIFrameWnd* pParent = (CMDIFrameWnd*)CWnd::FromHandle( phParent );
	pParent->m_hWndMDIClient = GetMainWnd()->GetSafeHwnd();

	if ( !CMDIChildWnd::Create( NULL, strCaption, WS_CHILD|WS_VISIBLE|WS_CLIPCHILDREN, rect, pParent ) )
		return FALSE;
