...
CSkinWindow* CSkin::GetWindowSkin(CWnd* pWnd)
{
	...
	#endif
	{
		...
		if ( strClassName == L"CPluginWnd" )
		{
			if ( CPluginWnd* pWindow = reinterpret_cast< CPluginWnd* >( pWnd ) )
			{
				CComBSTR bsName; 
				try
				{
					pWindow->m_pOwner->GetWndClassName( &bsName );
					strClassName = bsName;
				}
				catch (CException* e)
				{
		...
		if ( bPanel )
		{
			CSkinWindow* pSkin = GetWindowSkin( strClassName, _T(".Panel") );
		...
		for ( int nSuffix = 0 ; nSuffix < 3 && pszModeSuffix[ nSuffix ] != NULL ; nSuffix ++ )
		{
			...
			{
				CSkinWindow* pSkin = GetWindowSkin( strClassName, pszModeSuffix[ nSuffix ] );
				if ( pSkin != NULL ) return pSkin;
			}
		}
	}
